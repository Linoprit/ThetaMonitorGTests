/*
 * ID_Table.cpp
 *
 *  Created on: 11.11.2018
 *      Author: harald
 */

#include "ID_Table.h"
#include <stddef.h>


ID_Table::ID_Table() {
	// NOP
}

std::string ID_Table::sensorType2Str(uint8_t sensorType) {
	return sensorType2Str(static_cast<SensorType>(sensorType));
}
std::string ID_Table::sensorType2Str(SensorType sensorType) {
	std::string sensTypeStr;
	if (sensorType == ID_Table::SensorType::TEMP) {
		sensTypeStr = "TEMP";
	} else if (sensorType == ID_Table::SensorType::HUMIDITY) {
		sensTypeStr = "HUMI";
	} else if (sensorType == ID_Table::SensorType::PRESS) {
		sensTypeStr = "PRES";
	}
	return sensTypeStr;
}

void ID_Table::setSensId(Theta_sens_type sensVals) {

	//...
	// TODO write content to EEPROM, renew existing values, if different
	//..

}

void ID_Table::setStationId(uint32_t stationID) {

	//...
	// TODO write content to EEPROM, renew existing id, if different
	//..

}

void ID_Table::getSensIdTableE2(void) {
	//...
	// TODO get all IDs from E2 and print them
	//..

}

const uint32_t ID_Table::getStationId(void) {
	uint8_t result = 0;

	//...
	// TODO write content to EEPROM, renew existing id, if different
	//..

	return result;
}

/*
 const uint8_t ID_Table::get_stationNo(uint8_t sensor_ID[SENSOR_ID_LEN])
 {
 for(uint8_t i=0; i < ID_TABLE_LEN; i++)
 {
 if(ID_is_same(sensor_ID, (uint8_t*) theta_sensors_id_list[i].sensor_ID) )
 return theta_sensors_id_list[i].minTemp;
 }
 return INVLD_TEMPERATURE;
 }

 const uint8_t	ID_Table::get_relayNo(uint8_t sensor_ID[SENSOR_ID_LEN])
 {
 for(uint8_t i=0; i < ID_TABLE_LEN; i++)
 {
 if(ID_is_same(sensor_ID, (uint8_t*) theta_sensors_id_list[i].sensor_ID) )
 return theta_sensors_id_list[i].relayNo;
 }
 return INVLD_TEMPERATURE;
 }

 const theta_sens_type* ID_Table::get_struct(uint8_t sensor_ID[SENSOR_ID_LEN])
 {
 for(uint8_t i=0; i < ID_TABLE_LEN; i++)
 {
 if(ID_is_same(sensor_ID, (uint8_t*) theta_sensors_id_list[i].sensor_ID) )
 return &theta_sensors_id_list[i];
 }
 return NULL;
 }

 const theta_sens_type* get_struct(uint8_t index)
 {
 return &theta_sensors_id_list[index];
 }

 const char* ID_Table::get_shortname(uint8_t sensor_ID[SENSOR_ID_LEN])
 {
 for(uint8_t i=0; i < ID_TABLE_LEN; i++)
 {
 if(ID_is_same(sensor_ID, (uint8_t*) theta_sensors_id_list[i].sensor_ID) )
 return theta_sensors_id_list[i].shortname;
 }
 return NULL;
 }


 const bool	ID_Table::ID_is_same(uint8_t ID1[SENSOR_ID_LEN], uint8_t ID2[SENSOR_ID_LEN])
 {
 for (uint8_t i=0; i < SENSOR_ID_LEN; i++)
 {
 if(ID1[i] != ID2[i])
 return false;
 }
 return true;
 }
 */
